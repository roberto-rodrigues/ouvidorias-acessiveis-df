...
function render(){
 ...
 list.forEach((f,i)=>{const p=f.properties,[x,y]=f.geometry.coordinates;
  const m=L.marker([y,x],{icon:pinoIcon(p),riseOnHover:true}).bindPopup(popup(p,y,x),{autoPan:false,maxWidth:330}).addTo(markers);m.on('click',(e)=>{L.DomEvent.stopPropagation(e);destacarRA(p.RA);if(map.getZoom()<16){mostrarOuvidoria(m,y,x)}});
